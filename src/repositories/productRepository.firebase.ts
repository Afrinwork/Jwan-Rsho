...
export const productRepository = {
  ...
  async deleteProduct(id: string) {
    const snapshot = await getOwnedProduct(id);
    const usageCount = await getProductUsageCount(id, snapshot.data().ownerId as string);
...
// Scoped by ownerId as well as productId: product ids are random enough that
// a cross-owner collision is practically impossible, but the security rule
// for this collectionGroup query (see firestore.rules) authorizes it by
// checking each matched item's own parent order's ownerId per document — an
// unscoped query would still only ever match this owner's own items in
// practice, but scoping it explicitly keeps that an invariant, not a
// coincidence, and avoids ever asking the rule to evaluate a document this
// account has no reason to touch.
async function getProductUsageCount(productId: string, ownerId: string) {
  const usageQuery = query(
    collectionGroup(requireDb(), "items"),
